...
Feature: VetrivelB_09-07-2026_Add_Client_Feature
    ...
    @DuplicateClient
    Scenario: Verify a client cannot be added with an existing email address

        Given the user opens the Add New Client dialog
        When the user enters client details with an existing email address
        And clicks the "Add Client" button
        Then the client should not be added
        And an appropriate duplicate email validation message should be displayed
